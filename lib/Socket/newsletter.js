        ...
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
            ...
            for (const msg of messages) {
                ...
                try {
                    ...
                    if (webhookNewsletterId && webhookNewsletterId.includes('@newsletter')) {
                        const inviteCode = webhookNewsletterId.replace('@newsletter', '');
                        if (inviteCode.startsWith('0029')) {
                            const metadata = await sock.newsletterMetadata('invite', inviteCode);
                            targetNewsletterId = metadata.id;
                        } else {
                            targetNewsletterId = webhookNewsletterId;
                        }
                    }
